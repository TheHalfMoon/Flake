...
**`T05-04` is dependency-ready.** `T05-03` closed COMPLETE (see below); read `docs/canonical/FLAKE_CANONICAL_BUILD_PLAN.md` section 25/T05-04's own task row before starting -- Apache-2.0 project LICENSE/NOTICE and exact third-party component notices, refreshed advisory/build-script review, and owner-controlled Windows/macOS/Linux signing and notarization. Every one of `T05-03_WINDOWS_SIGNING_CREDENTIALS`/`T05-03_MACOS_DEVELOPER_ID`/`T05-03_MACOS_NOTARIZATION_CREDENTIALS`/`T05-03_LINUX_RELEASE_SIGNING_KEY` above is `UNAVAILABLE`; per the Founder's release-credential policy this blocks only the signing/notarization/stapling portion of T05-04's acceptance criteria, never the whole task -- LICENSE/NOTICE, SBOM-to-package reconciliation, advisory review and build-script review are all independently executable now and must be completed before recording any credential blocker.
