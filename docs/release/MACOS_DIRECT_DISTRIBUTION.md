...
Full design for Flake's macOS release path under
...
Flake distributes an ad-hoc-signed `.dmg` directly from its own README/docs and, once a
...
- ad-hoc-codesigned (`codesign --sign -`), which lets `codesign --verify` confirm the bundle's
  ...
  exactly that;
- checksummed (SHA-256);
- signed with Flake's own project-controlled GPG release-signing identity (the same key
  already qualified for Linux artifacts in `T05-04`, fingerprint
  `F779807C73F29F4DB1E7DC9F78F7D4B92287FE22`) — this proves the artifact came from the Flake
...
message resembling *"Flake" cannot be opened because it is from an unidentified developer* or,
on current macOS versions, *Apple could not verify "Flake" is free of malware*. This is
Gatekeeper working correctly against an unnotarized binary — not a bug in Flake's build, and
not something this project's CI or documentation works around.

### Opening Flake anyway (Apple's own supported per-app override)

1. Try to open Flake normally (double-click the `.app`, or open the mounted `.dmg` and drag
...
2. Open **System Settings → Privacy & Security**, scroll to the **Security** section, and
   click **Open Anyway** next to the message naming Flake. (On older macOS versions: right-click
...
Terminal command, and no system-wide security change, is required or recommended. Flake's own
...
4. Signs the `.dmg` and its manifest with Flake's project GPG release-signing key
...
published Flake release-signing public key, and optionally verify the GitHub attestation —
