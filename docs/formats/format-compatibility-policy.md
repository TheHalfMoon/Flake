...
rely on across every future Pluma release, so that promise is reviewable
...
|---|---|---|
| "current major epoch and its immediately previous epoch may be supported by the live reader" | `pluma-migrate`/`flake-migrate` (this task) reads format-1 nonmutating (`crate::vault::Vault::open_read`/`scan`) purely to migrate it into a fresh format-2 root; no live format-2 write path ever opens a format-1 root or vice versa — the two epochs are never silently blended in one live vault | `docs/formats/legacy-migration.md`; this task's own evidence |
...
| "Publish permanent format specifications, golden fixtures and versioned standalone offline migration tools" | This document plus `docs/formats/format-2-canonical-sqlite.md`/`docs/formats/typed-records.md`/`docs/formats/legacy-migration.md`; `tests/fixtures/migration/` and `tests/fixtures/format-compat/`; `pluma-migrate`/`flake-migrate` (`src/bin/migrate.rs`) | This task |
| "old owned data must remain readable without keeping an obsolete main app" | `pluma-migrate`/`flake-migrate` links only the `fehrest` library crate, builds and runs standalone, offline, independent of the desktop app's own release cadence (see its own module docs for the exact reasoning) | This task |
...
`pluma-migrate` (`src/bin/migrate.rs`, this task) is a thin CLI/JSON
...
any) is installed. `flake-migrate` is the deprecated compatibility alias
kept after the 2026-09-20 Flake→Pluma product rename (same `[[bin]]`
source path, `Cargo.toml`); it will not be removed without a separate,
explicitly governed breaking release:

```sh
cargo build --release --bin pluma-migrate
./target/release/pluma-migrate preview <format-1-vault-root>
./target/release/pluma-migrate import  <format-1-vault-root> <new-format-2-root>
```
