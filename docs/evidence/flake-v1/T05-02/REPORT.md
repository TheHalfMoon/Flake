...
### A second harness gap, found by the CI cross-platform D6 run: read-only open cannot roll back a hot journal

The `d6-vm-unclean-shutdown-linux` CI job (GitHub-hosted `ubuntu-latest`, independent of this session's own WSL2 host) reproduced a real hot-journal-after-kill scenario on several of its own cycles and surfaced a second, genuine gap in this harness's own verification step — not the 30-cycle Linux run above, which happened not to hit this exact window, but confirmed as real and reproducible once investigated directly against this repository's own product code:

`verify_fault_cycle.py` failed with `OperationalError: attempt to write a readonly database` on several CI cycles. Root cause: `tools/independent-verify/sqlite_reader.py`'s own reader deliberately opens every database strictly read-only (`?mode=ro`) — correct and unchanged for reading a live vault, since it must never touch what it inspects — but SQLite cannot perform its own standard hot-journal rollback on a connection opened with that flag; officially documented SQLite behavior, and the exact error text SQLite itself produces for precisely this case. A real Flake process's own next open (via a normal, non-read-only connection) rolls the same journal back transparently and correctly, exactly as intended by `journal_mode=DELETE`/`synchronous=EXTRA`'s own crash-safety design — only this harness's own read-only verification step, not the product, was affected.

**Confirmed directly**, not merely inferred from the CI log: forced a real hot journal locally (a slow multi-page SQLite write, killed mid-transaction via `SIGKILL`) and reproduced the identical `OperationalError` against the unmodified reader. **Fixed** in `verify_fault_cycle.py` alone (never in the shared, already-reviewed `sqlite_reader.py`, which no `flake-v1` task should casually modify): before the read-only independent read, briefly open the same disposable copy read-write and issue one statement, letting SQLite complete its own built-in crash recovery on a file this script owns and is free to touch — then the read-only reader opens a clean, journal-free database, exactly as it does after every one of the earlier successful cycles. Re-verified against the same reproduced hot-journal fixture: `ok: true`, `command_count: 10` — the interrupted write correctly rolled all the way back, not partially applied, matching `journal_mode=DELETE`'s own all-or-nothing guarantee.

This is the second, and final, harness-only gap this task's own D6 work surfaced (the first being the missing `-journal` file copy in `verify_cycle`, fixed earlier in this same evidence). Both gaps were in this new, disposable verification tooling — never in the product's own commit/recovery code, which behaved correctly and consistently at every single cycle across all three exercises (the original 30-cycle Linux run, this reproduction, and CI's own run).
