...
`.github/workflows/t05-03-release-candidates.yml` ran green on all three native profiles on PR #99's final commit (run `35179292875` + companion inherited-gate runs `35179292876`/`35179292885`/`35179292934`/`35179292942`, all triggered by the same commit): `cli-archive-and-sbom`, `desktop-bundle-install-test`, and `reproducibility` all passed on `windows-latest`/`macos-latest`/`ubuntu-latest`, alongside every inherited full gate re-run against this same commit (`qualify` ×3 OS from `T04-06`'s workflow, `T05-01`'s migration qualification, `T05-02`'s durability/performance qualification including `d6-vm-unclean-shutdown-linux` and `m-scale-performance`, and `verify-artifacts`).

**A real defect was found and fixed during this qualification, not merely observed:** the Windows `desktop-bundle-install-test` job hung indefinitely (one run cancelled after 48+ minutes; two more runs added a step-level timeout and then a corrected process-tree diagnostic, each still timing out at their bound) on `install_test.sh`'s very first NSIS installer invocation. Two plausible root-cause theories were checked directly against the actual CI runner and ruled out with real evidence, not assumption: (a) the WebView2-runtime-install/elevation path in Tauri's own NSIS template — a registry diagnostic confirmed this runner's WebView2 Runtime registration IS present under the exact key that template checks, so that whole branch never executes; (b) Windows Defender's execution-time scan of the large unsigned installer — disabling real-time monitoring for the job made no difference. The actual cause, confirmed by capturing the installer's own live argv during the hang: this project's `install_test.sh` runs under MSYS bash, which silently rewrites a bare `/S` argument into a Windows drive-relative path (`S:/`) before the native child process ever sees it (confirmed both by the captured argv — `Flake_..._x64-setup.exe S:/ /D=...`, never `/S /D=...` — and by a local reproduction of the exact same conversion outside CI). Silent mode was therefore never actually requested; the installer launched its normal interactive GUI wizard, which then waits forever for a click no headless CI session can ever provide. Fixed with `//S` (MSYS's standard escape for exactly this class of bug, already used elsewhere in this same script for `taskkill`'s `/F`/`/T`/`/PID` flags), verified locally against a real native process before pushing. The bounded per-invocation timeout and live process-tree diagnostic added while chasing this (`run_windows_exe_with_diagnostics`, using GNU `timeout` rather than a hand-rolled `kill -0` poll — an earlier version of that same helper was itself found to give a false "hang" positive under MSYS's zombie-PID semantics, corrected before being relied on) are kept as a permanent defensive safety net for any future CI hang on this job, now with a correct, non-buggy detector.
...
"Every unsigned candidate installs/runs/updates/uninstalls on clean network-blocked native image with retained vault bytes and complete dependency inventory" — met for the unsigned-candidate scope this task's own acceptance clause and the Founder's ruling both define, subject to Limitations 1-5 above, now that the CI results section above is green on all three native profiles. Signing/notarization/LICENSE/NOTICE remain explicitly deferred to `T05-04` per that same acceptance clause's own "unsigned candidate" wording.
